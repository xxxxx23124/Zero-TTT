"""Typed inspection and recovery for console-owned model artifacts."""
...
class ArtifactCoordinator:
    ...
    def _publish_checkpoint(self, checkpoint: LoadedCheckpoint) -> Path:
        ...
        actual = PublicationSummary.from_payload(
            self.manager.load_publication(publication)
        )
    ...
    def reconcile(
        ...
        if checkpoint is None:
            if inspection.publication_error:
                raise ValueError(
                    f"current publication is invalid: {inspection.publication_error}"
                )
        ...
        if publication is not None and publication.identity.run_id == desired.run_id:
            ...
            if actual.optimizer_step > desired.optimizer_step:
                raise ValueError(
                    "publication is ahead of the latest resumable checkpoint"
                )
            if (
                actual.optimizer_step == desired.optimizer_step
                and actual != desired
            ):
                raise ValueError(
                    "publication conflicts with the latest resumable checkpoint"
                )
    ...
    def publish_learner(
        ...
        summary = PublicationSummary.from_payload(
            self.manager.load_publication(publication_path)
        )
