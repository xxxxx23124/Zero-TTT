...
def stable_game_id(
    ...
    return sha256_bytes(canonical_json_bytes([dataset_id, asset_sha256, member_path, ordinal]))
...
def _float32_tuple(values: tuple[float, ...]) -> tuple[float, ...]:
    return tuple(float(value) for value in np.asarray(values, dtype=np.float32))


def _validate_distribution(
    values: tuple[float, ...], negative_message: str, sum_message: str
) -> None:
    if not values or any(value < 0 or not math.isfinite(value) for value in values):
        raise ValueError(negative_message)
    if abs(sum(values) - 1.0) > 1e-5:
        raise ValueError(sum_message)
...
class TrajectoryRecord:
    ...
    def __post_init__(self) -> None:
        ...
        self._validate_identity_and_moves()
        self._fill_search_defaults()
        self._validate_policy_shape()
        self._normalize_float_targets()
        self._validate_targets()
        self._validate_search_metadata()
        self._validate_content_identity()

    def _validate_identity_and_moves(self) -> None:
        message = "game and asset identities must be SHA-256 hex strings"
        _require_sha256_hex(self.game_id, message)
        _require_sha256_hex(self.asset_sha256, message)
        if self.max_moves < 2:
    ...
    def _fill_search_defaults(self) -> None:
    ...
    def _validate_policy_shape(self) -> None:
        ...
        if any(
            a > b
            for a, b in zip(self.policy_row_offsets, self.policy_row_offsets[1:], strict=False)
        ):
    ...
    def _normalize_float_targets(self) -> None:
        object.__setattr__(self, "policy_values", _float32_tuple(self.policy_values))
        ...
        object.__setattr__(self, "ownership_black", _float32_tuple(self.ownership_black))
        object.__setattr__(self, "root_values", _float32_tuple(self.root_values))
        object.__setattr__(self, "root_score_margins", _float32_tuple(self.root_score_margins))
        object.__setattr__(self, "temperatures", _float32_tuple(self.temperatures))

    def _validate_targets(self) -> None:
        for start, end in zip(self.policy_row_offsets, self.policy_row_offsets[1:], strict=False):
            _validate_distribution(
                self.policy_values[start:end],
                "each policy row must be a non-negative distribution",
                "each policy row must sum to one",
            )
        ...
        if not all(
            ...
            for value in (
                ...
                *self.root_values,
                *self.root_score_margins,
                *self.temperatures,
            )
        ):
            raise ValueError("trajectory targets must be finite")

    def _validate_search_metadata(self) -> None:
        ...
        if self.source_kind == "selfplay/mcts":
            ...
            if any(not value for value in identities) or not all(self.search_metadata_mask):
                raise ValueError("MCTS self-play trajectories require complete identities")

    def _validate_content_identity(self) -> None:
...
class AnnotationRecord:
    ...
    def __post_init__(self) -> None:
        RECORD_SCHEMA.require(self.schema_version)
        _require_sha256_hex(self.game_id, "annotation identity is invalid")
        if self.ply < 0 or not self.teacher_fingerprint:
            raise ValueError("annotation identity is invalid")
        if len(self.policy_actions) != len(self.policy_values):
            raise ValueError("annotation policy actions and values disagree")
        normalized_policy = _float32_tuple(self.policy_values)
        normalized_ownership = _float32_tuple(self.ownership)
        ...
        if normalized_policy:
            _validate_distribution(
                normalized_policy,
                "annotation policy cannot contain negative mass",
                "annotation policy must sum to one",
            )
        if any(not 0 <= action < ACTION_SIZE for action in self.policy_actions):
        ...
        if not all(
            math.isfinite(value) for value in (self.value, self.score_margin, *self.ownership)
        ):
...
class ImportEvent:
    ...
    def __post_init__(self) -> None:
        ...
        elif self.kind == "annotation" and (
            not isinstance(self.record, AnnotationRecord) or self.reason_code is not None
        ):
