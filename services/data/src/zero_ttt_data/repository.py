...
from zero_ttt_dataset.locators import ImportStatistics, SelfPlayStatistics
...
class CatalogRepository:
    ...
    def commit_trajectory_shard(
        ...
        with self.connection:
            ...
            for game_index, record in enumerate(records):
                existing = self.connection.execute(
                    "SELECT content_sha256,asset_sha256,shard_sha256,game_index,"
                    "trainable_start_ply,trainable_positions FROM trajectories WHERE game_id=?",
                    (record.game_id,),
                ).fetchone()
                expected = (
                ...
                if existing is not None:
                    if tuple(existing) != expected:
                        raise ValueError("trajectory identity conflicts with admitted content")
                    continue
                self.connection.execute(
                    """
    ...
    def commit_annotation_shard(self, info: ShardInfo, records: list[AnnotationRecord]) -> None:
        ...
        with self.connection:
            ...
            for record_index, record in enumerate(records):
                existing = self.connection.execute(
                    "SELECT content_sha256,shard_sha256,record_index FROM annotations "
                    "WHERE game_id=? AND ply=? AND teacher_fingerprint=?",
                    (record.game_id, record.ply, record.teacher_fingerprint),
                ).fetchone()
                expected = (record.content_sha256, info.sha256, record_index)
                if existing is not None:
                    if tuple(existing) != expected:
                        raise ValueError("annotation identity conflicts with admitted content")
                    continue
                self.connection.execute(
                    """
    ...
    def _insert_shard(self, info: ShardInfo) -> None:
        existing = self.connection.execute(
            "SELECT kind,relative_path,size_bytes,record_count,position_count "
            "FROM shards WHERE shard_sha256=?",
            (info.sha256,),
        ).fetchone()
        expected = (
            info.kind,
        ...
        if existing is not None:
            if tuple(existing) != expected:
                raise ValueError("shard identity conflicts with existing metadata")
            return
        self.connection.execute(
            """
            INSERT INTO shards(
