...
状态：v2 SGF trajectory/annotation、NPZ shard、SQLite catalog 和 snapshot 已实现。采用类似
...
- `game_id`、schema、规则、贴目、`max_moves`、终止原因和结果；v2 固定空棋盘黑方先行；
...
- v1 catalog/shard 不迁移；打开时明确拒绝，必须从 manifest 重新导入并创建 v2 snapshot。
